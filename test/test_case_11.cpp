...
{
	MemoryMappedIO memoryMappedIO(L"test_case_11.bin");
	...
	double out[outSize];
	MemoryMappedIO inputFile(L"test_case_11_in.bin");
	...
	BlockMatch<double> blockMatch(typeid(uint8_t), typeid(uint8_t), typeid(double), typeid(uint8_t),
		...
		BorderType::normal, SearchFrom::center, true,
	...
	MemoryMappedIO indexFile(L"test_case_11_ind.bin");
