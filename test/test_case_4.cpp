...
BOOST_AUTO_TEST_CASE(test_case_4)
{
	...
	int matM = 10, matN = 10, blockM = 2, blockN = 2, strideM = 2, strideN = 2,
	...
	BlockMatch<float> match(typeid(float), typeid(float), typeid(float), typeid(int),
		SearchType::global, MeasureMethod::mse, PadMethod::symmetric, PadMethod::symmetric, BorderType::normal, 
		true,
