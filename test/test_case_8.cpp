...
BOOST_AUTO_TEST_CASE(test_case_8)
{
	...
	BlockMatch<float> match(typeid(float), typeid(float), typeid(float), typeid(nullptr),
		SearchType::global, MeasureMethod::mse, PadMethod::symmetric, PadMethod::symmetric, BorderType::normal, false,
		matM, matN, matM, matN, numberOfChannels,
	...
	float *matrixAPadded = (float*)malloc(matrixA_padded_M * matrixA_padded_N * numberOfChannels * sizeof(float));
	float *matrixBPadded = (float*)malloc(matrixB_padded_M * matrixB_padded_N * numberOfChannels * sizeof(float));
