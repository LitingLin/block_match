...
bool validate_pad_result(float *to_validate, float *truth_array, int src_m, int src_n, int padding_m, int padding_n, int index_m, int index_n, int block_m, int block_n)
{
	...
	for (int i = 0; i < block_m; i++)
		for (int j = 0; j < block_n; j++)
			if (to_validate[i*block_m + j] != truth_array[(truth_index_m + i)*src_m + truth_index_n + j])
				return false;
	return true;
}
...
BOOST_AUTO_TEST_CASE(test_padding)
{
	...
	BOOST_CHECK(validate_pad_result(buf, padded, 6, 7, 2, 2, test_index_m, test_index_n, test_block_m, test_block_n));
	test_index_m = -2;
	test_index_n = -1;
	test_block_m = 1;
	test_block_n = 3;
